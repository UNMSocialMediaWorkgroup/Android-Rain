...
public class DatabaseHandler extends SQLiteOpenHelper {
 ...
 // Adding new RESPONSE
    void addResponse(Response response) {
        ...
        values.put(KEY_SUFFIX, response.getSuffix()); 
        values.put(KEY_ANSWER, response.getAnswer()); 
        values.put(KEY_RANGE, response.getRange()); 
        // Inserting Row
    ...
    Response getResponse(int i) {
        ...
        Cursor cursor = db.query(TABLE_RESPONSES, new String[] {
        		KEY_ID, KEY_TEXT, KEY_SUFFIX, KEY_ANSWER,KEY_RANGE,}, KEY_ID + "=?",
        ...
        if (cursor.moveToFirst())
        {
            ...
            response.setSuffix(			cursor.getString(2));
            response.setAnswer(         cursor.getInt(3));
            response.setRange(          cursor.getInt(4));
            // return response
            cursor.close();
            db.close();
            return response;
